...
async function submitForm(e, form){
  ...
  const response = await fetchService.performPostHttpRequest(AUTH_ENDPOINT+"update_email_address", headers, jsonFormData);
  ...
  if(statusType === "2"){
    ...
    // Notify user of the success, then ask them to check inbox.
    alertPopup.createAlertPopup("success", response.message);
    setTimeout(() => {
      window.location.href = "/accounts/email/account-created-success";}, 1000);
  }
  // Client error
...
function validateForm(form){
  ...
  const errorMessage = formUtils.validateForm(form, ["username","password","new_email_address"]);
  if(errorMessage){

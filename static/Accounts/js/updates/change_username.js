...
async function submitForm(e, form){
  ...
  const response = await fetchService.performPostHttpRequest("https://auth.pyroneon.net:8443/api/update_username", headers, jsonFormData);
...
function validateForm(form){
  ...
  const errorMessage = formUtils.validateForm(form, ["username","password","new_username"]);
  if(errorMessage){
    // Return the error message.
    return errorMessage;
  }
  if(form.username === form.new_username){
    return "New username can't be the same as old username.";
  }
